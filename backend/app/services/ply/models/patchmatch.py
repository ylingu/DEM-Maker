...
3. Evaluation: compute the matching costs for all the hypotheses and 
...
class DepthInitialization(nn.Module):
    ...
    def forward(
        ...
        Returns:
            depth_sample: initialized sample depth map by randomization or 
            local perturbation (B, Ndepth, H, W)
        """
        ...
        if is_empty(depth):
            ...
            depth_sample = inverse_max_depth.view(batch_size, 1, 1, 1) 
            ...
            depth_sample = (
                torch.arange(-self.patchmatch_num_sample // 2, 
                             self.patchmatch_num_sample // 2, 1, device=device)
                .view(1, self.patchmatch_num_sample, 1, 1).repeat(batch_size, 1, 
                                                                  height, width).float()
            )
            inverse_depth_interval = (inverse_min_depth 
                                      - inverse_max_depth) * depth_interval_scale
            ...
            for k in range(batch_size):
                depth_clamped.append(
                    torch.clamp(depth_sample[k], min=inverse_max_depth[k], 
                                max=inverse_min_depth[k]).unsqueeze(0)
                )
...
class Propagation(nn.Module):
    ...
    def forward(self, depth_sample: torch.Tensor, grid: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            depth_sample: sample depth map, in shape of
              [batch, num_depth, height, width],
            grid: 2D grid for bilinear gridding, in shape of [batch, neighbors*H, W, 2]

        Returns:
            propagate depth: sorted propagate depth map 
...
class Evaluation(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            src_features: features from (Nview-1) source views, (Nview-1) 
            * (B, C, H, W),
              where Nview is the number of
            ...
            where Nview is the number of input
            ...
            view_weights: Tensor to store weights of source views, 
            in shape of (B,Nview-1,H,W),
        ...
        Returns:
            ...
            view_weights: optional, Tensor to store weights of source views, 
            in shape of (B,Nview-1,H,W),
                Nview-1 represents the number of source views
        """
        ...
        pixel_wise_weight_sum = 1e-5 * torch.ones((batch, 1, 1, height, width), 
                                                  dtype=torch.float32, device=device)
        ref_feature = ref_feature.view(batch, self.G, feature_channel // self.G, 1, 
                                       height, width)
        similarity_sum = torch.zeros((batch, self.G, num_depth, height, width), 
                                     dtype=torch.float32, device=device)
        ...
        for src_feature, src_proj in zip(src_features, src_projs):
            ...
            if is_empty(view_weights):
                ...
                # reuse the pixel-wise view weight from first iteration of 
                # Patchmatch on stage 3
        ...
        if is_empty(view_weights):
            ...
            view_weights = torch.cat(view_weights_list, dim=1) 

        if is_inverse:
            # depth regression: inverse depth regression
            depth_index = torch.arange(0, num_depth, 1, device=device).view(1,
                                                                            num_depth,1,1)
            depth_index = torch.sum(depth_index * score, dim=1)

            inverse_min_depth = 1.0 / depth_sample[:, -1, :, :]
            inverse_max_depth = 1.0 / depth_sample[:, 0, :, :]
            depth_sample = inverse_max_depth 
            + depth_index / (num_depth - 1) * (inverse_min_depth - inverse_max_depth)
...
class PatchMatch(nn.Module):
    ...
    def __init__(
        ...
        # Using dictionary instead of Enum since TorchScript cannot recognize 
    ...
    def get_grid(
        self, grid_type: int, batch: int, height: int, width: int, 
        offset: torch.Tensor, device: torch.device
    ) -> torch.Tensor:
        """Compute the offset for adaptive propagation or spatial 
        cost aggregation in adaptive evaluation
