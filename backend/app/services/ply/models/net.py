...
class FeatureNet(nn.Module):
    ...
    of original images from each view"""
    ...
    def forward(self, x: torch.Tensor) -> dict[int, torch.Tensor]:
        ...
        Returns:
            output_feature: a python dictionary contains extracted features 
        ...
        intra_feat = F.interpolate(conv10, scale_factor=2.0, mode="bilinear", 
                                   align_corners=False) + self.inner1(conv7)
        del conv7
        del conv10

        output_feature[2] = self.output2(intra_feat)
        intra_feat = F.interpolate(
            intra_feat, scale_factor=2.0, mode="bilinear", align_corners=False) 
        + self.inner2(conv4)
        del conv4
...
class Refinement(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv = nn.ConvTranspose2d(
            in_channels=8, out_channels=8, kernel_size=3, padding=1, output_padding=1, 
            stride=2, bias=False
        )

        self.bn = nn.BatchNorm2d(8)
        self.conv3 = ConvBnReLU(in_channels=16, out_channels=8)
        self.res = nn.Conv2d(in_channels=8, out_channels=1, kernel_size=3, padding=1, 
                             bias=False)

    def forward(
        self, img: torch.Tensor, depth_0: torch.Tensor, depth_min: torch.Tensor, 
...
class PatchmatchNet(nn.Module):
    ...
    def forward(
        ...
        ref_image = images[0]
        ref_feature, src_features = self.extract_features(images)
        depth_patchmatch, depth = self.perform_patchmatch(
            ref_feature, src_features, intrinsics, extrinsics, depth_min, depth_max
        )
        depth = self.refine_depth(depth, ref_image, depth_min, depth_max, orig_height, orig_width)
        depth_patchmatch[0] = [depth]

        if self.training:
            return depth, torch.empty(0, device=intrinsics.device), depth_patchmatch
        else:
            return self.compute_confidence(depth, depth_patchmatch, orig_height, orig_width)

    def extract_features(self, images: list[torch.Tensor]) -> tuple[dict[int, torch.Tensor], list[dict[int, torch.Tensor]]]:
        features = [self.feature(img) for img in images]
        return features[0], features[1:]

    def perform_patchmatch(
        self,
        ref_feature: dict[int, torch.Tensor],
        src_features: list[dict[int, torch.Tensor]],
        intrinsics: torch.Tensor,
        extrinsics: torch.Tensor,
        depth_min: torch.Tensor,
        depth_max: torch.Tensor,
        ...
        depth_patchmatch = {}
        depth = torch.empty(0, device=intrinsics.device)
        scale = 0.125

        for stage in range(self.stages - 1, 0, -1):
            depth, depth_patchmatch = self.run_stage(
                stage, ref_feature, src_features, intrinsics, extrinsics, depth_min, depth_max, depth, depth_patchmatch, scale
            )
            scale *= 2.0
        return depth_patchmatch, depth
    ...
    def run_stage(
        ...
        intrinsics_l, proj_l = self.create_projection_matrices(intrinsics, extrinsics, scale)
        ref_proj, src_proj = proj_l[0], proj_l[1:]

        patchmatch = getattr(self, f"patchmatch_{stage}")
        depths, score, view_weights = patchmatch(
        ...
        return depths[-1].detach(), depth_patchmatch

    def create_projection_matrices(
        self, intrinsics: torch.Tensor, extrinsics: torch.Tensor, scale: float
        ...
        return intrinsics_l, torch.unbind(proj, 1)
    ...
    def refine_depth(
        ...
        if depth.size(2) != orig_height or depth.size(3) != orig_width:
            depth = F.interpolate(depth, size=[orig_height, orig_width], mode="bilinear", align_corners=False)
        return depth

    def compute_confidence(
        self, depth: torch.Tensor, depth_patchmatch: dict[int, list[torch.Tensor]], orig_height: int, orig_width: int
    ) -> tuple[torch.Tensor, torch.Tensor, dict[int, list[torch.Tensor]]]:
        num_depth = self.patchmatch_num_sample[0]
        score_sum4 = 4 * F.avg_pool3d(
            F.pad(depth_patchmatch[3][0].unsqueeze(1), pad=(0, 0, 0, 0, 1, 2)), (4, 1, 1), stride=1, padding=0
        ).squeeze(1)
        depth_index = depth_regression(
            depth_patchmatch[3][0], depth_values=torch.arange(num_depth, device=depth.device, dtype=torch.float)
