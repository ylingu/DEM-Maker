...
def calc_score(ind1: int, ind2: int, cam_center: List[np.ndarray], points3d: Dict[int, Point3D]) -> float:
    id_intersect = set(images[ind1].point3d_ids) & set(images[ind2].point3d_ids)
    id_intersect = [pid for pid in id_intersect if pid != -1]
    if not id_intersect:
        return 0.0

    points = np.array([points3d[pid].xyz for pid in id_intersect])
    vec_i = cam_center[ind1] - points
    vec_j = cam_center[ind2] - points
    norms_i = np.linalg.norm(vec_i, axis=1)
    norms_j = np.linalg.norm(vec_j, axis=1)
    cos_theta = np.einsum('ij,ij->i', vec_i, vec_j) / (norms_i * norms_j)
    cos_theta = np.clip(cos_theta, -1.0, 1.0)  # 防止浮点误差
    theta = (180 / np.pi) * np.arccos(cos_theta)
    sigma = np.where(theta <= args.theta0, args.sigma1, args.sigma2)
    view_score_ = np.sum(np.exp(-(theta - args.theta0) ** 2 / (2 * sigma ** 2)))
    return view_score_
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(args.input_folder, "sparse/0")
    ...
    # 计算相机中心
    cam_center = [
        -np.matmul(extrinsic[:3, :3].T, extrinsic[:3, 3:4])[:, 0]
        for extrinsic in [
            np.vstack((np.hstack((quaternion_to_rotation_matrix(img.qvec), np.array(img.tvec).reshape(3, 1))),
                       [0, 0, 0, 1]))
            for img in images
        ]
    ]
    ...
    queue = [(i, j) for i in range(num_images) for j in range(i + 1, num_images)]

    def calc_score_wrapper(pair):
        i, j = pair
        return i, j, calc_score(i, j, cam_center, points3d)

    with ThreadPoolExecutor() as executor:
        for i, j, s in executor.map(calc_score_wrapper, queue):
            score[i, j] = s
            score[j, i] = s
            print("score[%d, %d] = %f" % (i, j, s))
    print("score[0]\n", score[0], end="\n\n")
